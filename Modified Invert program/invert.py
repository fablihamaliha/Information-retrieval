...
def arrange_words(words_list, idx):
    ...
    while '.B' not in words_list[idx]:
        ...
        arranged_words = arranged_words + " "
        idx += 1
    return arranged_words


# Function for arranging title
def arrange_title(words_list, idx):
    arranged_title = ""

    while True:
        if '.W' in words_list[idx] or \
           '.W\n' in words_list[idx] or \
            '.B' in words_list[idx] or \
            '.B\n' in words_list[idx] or \
            '.A' in words_list[idx] or \
            '.A\n' in words_list[idx] or \
            '.N' in words_list[idx] or \
            '.N\n' in words_list[idx] or \
            '.K' in words_list[idx] or \
            '.K\n'in words_list[idx] or \
            '.X' in words_list[idx] :
            break
        arranged_title = arranged_title + words_list[idx]
        arranged_title = arranged_title + " "
        idx += 1
    return arranged_title
...
def arrange_authors(words_list, idx):
    ...
    while words_list[idx] != ".N\n":
        ...
        arranged_authors = arranged_authors + " "
        idx = idx + 1
...
def process_cacm_data(cacm_lines, stop_input_user, stem_input_user, stop_words):
    ...
    for i in range(len(cacm_lines)):
        ...
        if ".I " in line:
            record_dict = dict.fromkeys(['I', 'T', 'OT', 'W', 'B', 'OA', 'A'])
            record_dict['I'] = cacm_lines[i][2:len(cacm_lines)]

        if ".T" in line:
            word_arr = arrange_title(cacm_lines, i + 1)
            record_dict['OT'] = word_arr
            if stop_input_user == 'n' and stem_input_user == 'n':
                record_dict['T'] = generate_tokens(word_arr)

            elif stop_input_user == 'y' and stem_input_user == 'n':
                record_dict['T'] = remove_stop_words(word_arr.lower(), stop_words)
        ...
        if ".B" in line:
            ...
            record_dict['B'] = cacm_lines[i+1][5:len(cacm_lines)]

        if ".A" in line:
            word_arr = arrange_authors(cacm_lines, i + 1)
            record_dict['OA'] = word_arr
            author_list = generate_tokens(word_arr)
...
# Function for creating the posting list associated with word index
